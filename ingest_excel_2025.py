"""
ingest_excel_2025.py — FedEx 2025 Rate Ingestion from Excel
-------------------------------------------------------
Parses FedEx_Standard_List_Rates_2025.xlsx directly.
100% accurate — no PDF parsing errors.

Sheets handled:
  1. "2025 U.S. rates"        — Domestic express (First Overnight, Priority, Standard, 2Day AM, 2Day, Express Saver)
  2. "2025 U.S. Export rates" — International export (Puerto Rico, Canada Zones A-C, Zones D-O)
  3. "2025 U.S. Import rates" — International import zones
  4. "2025 Ground & FHD rates"— FedEx Ground and Home Delivery

Plus: surcharge_chunks.py structured surcharges

Run:
    python ingest_excel.py --excel "FedEx_Standard_List_Rates_2025.xlsx"
"""
...
CHROMA_DIR  = "./chroma_fedex_db_2025"
...
def make_doc(service, zone, weight, price, page=0, direction=""):
    ...
    content = " ".join(parts)
    meta = {
        "source": "FedEx_Standard_List_Rates_2025",
        "page": page,
        "type": "rate",
        "service": service,
        "zone": zone,
...
def ingest(excel_path):
    ...
    try:
        from surcharge_chunks_2025 import SURCHARGE_CHUNKS
        surcharge_docs = [
            Document(
                page_content=chunk,
                metadata={"source": "FedEx_Service_Guide_2025", "page": 0, "type": "surcharge"}
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--excel", default="FedEx_Standard_List_Rates_2025.xlsx")
